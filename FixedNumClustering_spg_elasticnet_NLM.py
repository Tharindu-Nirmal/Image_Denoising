...
def non_local_means_rowwise(data_vectors, h=0.1):
    ...
    Parameters:
    data_vectors: an mxN array where each row is an N-dimensional data point.
    h: filtering parameter, controls the decay of the weights based on similarity.
    
    Returns:
    denoised_data: an mxN array, same shape as data_vectors, denoised version of the input data.
    """
    
    # Number of data points (rows)
    num_data_points, num_features = data_vectors.shape
    
    # Initialize the denoised output array
    denoised_data = np.zeros_like(data_vectors)
    
    # Compute pairwise distances between all data points (rows of data_vectors)
    distances = euclidean_distances(data_vectors, data_vectors)
    
    # Apply Non-Local Means denoising for each row
    for i in range(num_data_points):
        # Compute the weights based on the distance to all other rows
        weights = np.exp(-distances[i, :]**2 / (h**2))
