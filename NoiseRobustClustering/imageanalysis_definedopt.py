...
for i in range(N):
    ...
    if i % 10 == 0:
        ...
        print('L1 norm x', np.linalg.norm(beta_optimized, 1), file=output_file)
        print('sum of x', np.sum(beta_optimized), file=output_file)
        print('L2 norm Ax-b', np.linalg.norm(A @ beta_optimized - b, 2), file=output_file)
